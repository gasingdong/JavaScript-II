...
log("Full Names:", fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
const allCaps = runners.map(runner => runner.first_name.toLocaleUpperCase());
log("Uppercase First Names:", allCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
const largeShirts = runners.filter(runner => runner.shirt_size === "L");
log("Large Shirt Orders:", largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
const ticketPriceTotal = runners.reduce((totalDonation, runner) => totalDonation + runner.donation, 0);
log("Total Donations:", ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// The event director needs the names of all the companies in attendance with donations of over 200 dollars in order to identify the big donors
const moneyCompanies = runners.reduce((bigCompanies, runner) => {

    if (runner.donation > 200) {
        bigCompanies.push(runner.company_name);
    }
    return bigCompanies;
}, []);
log("Big Donor Companies:", moneyCompanies);

// Problem 2
// Due to a stocking error, there are only 5 3XL's available at the event. Find out how many 3XL's are needed.
const triplexl = runners.filter(runners => runners.shirt_size === "3XL");
log("3XL Shirt Order Amount:", triplexl.length); 

// Problem 3
// Find the largest donation amount and the smallest donation amount for data purposes.

// Need to pass in a comparator to sort because these are number comparisons
const donations = runners.map(runners => runners.donation).sort((a,b) => a-b);
log("Largest Donation:", donations[donations.length - 1]);
log("Smallest Donation:", donations[0]);